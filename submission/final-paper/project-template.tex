...
The paper-facing refresh verified four Neuronic summaries and two Google Drive context artifacts. The core PPO/SAC/TD3 claims come from \texttt{citylearn-72cell-lr-screen-20260506-r1} (504 rows) and \texttt{citylearn-final-hp-20260506-r1} (567 rows). MAPPO (126 rows) and residual SAC (189 rows) are retained as negative context only: their best released-all means are 0.961 and 1.008, respectively, so they are not competitive with PPO/SAC/TD3. The CHESCA reproduction comes from the Drive artifact \texttt{chesca\_upstream\_reproduction\_20260507.json}; it scores 0.480 on \texttt{public\_dev} and 0.508 on released phase 2, but it is an upstream optimization controller rather than our learned RL method. Exact source paths, Drive folders, and claim-to-CSV mappings are tracked in \texttt{submission/results/result\_source\_audit.csv} and \texttt{submission/results/paper\_claim\_map.csv}.
