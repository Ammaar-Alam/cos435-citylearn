...
\begin{itemize}
    \item SAC with a reward aligned to the official scoring terms reduces the RBC composite score by 48.4\% on the local evaluation dataset and by 40\% on the released held-out phase-2 splits; checkpoint ensembling further reduces the best SAC-family phase-2 score to 0.616.
...
\begin{figure}[h]
    ...
    \caption{
        Final shared-controller result matrix from the targeted PPO/SAC/TD3 sweep. Bars show the best setting per learned algorithm and evaluation group; error bars are 95\% confidence intervals over evaluation jobs. The CHESCA marker is an optimization-controller reference and is included as benchmark context, not as part of our learned RL method average.
    }
...
\begin{figure}[h]
    ...
    \caption{
        Broad learning-rate screen on the released phase-3 transfer splits. Each point aggregates three seeds over three 6-building splits. SAC is stable across mid-to-high learning rates, while TD3 has a sharper optimum at $3{\times}10^{-4}$ that gives the best phase-3 score in the broad sweep.
    }
    \label{fig:lr_phase3}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{
        \texttt{average\_score} (lower is better) on six released held-out evaluation datasets. The left panel shows three \texttt{phase\_2\_online\_eval} splits (3 buildings); the shaded right panel shows three \texttt{phase\_3} splits (6 buildings). Centralized policies cannot execute on phase-3 due to input/output size mismatch. SAC-DTDE achieves the best phase-3 mean (0.774), showing that shared-parameter decentralization enables cross-size evaluation. The dashed line is the CHESCA 2023 public leaderboard reference from a different evaluator and is shown only as benchmark context.
    }
    \label{fig:per_split}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{
        Method comparison on the three released \texttt{phase\_2\_online\_eval} splits. Bars aggregate the three held-out 3-building splits; error bars show 95\% confidence intervals over evaluation jobs. All learned methods beat the phase-2 RBC baseline, SAC variants lead, and the smoothness-regularized \texttt{reward\_v2} variant achieves the best single-checkpoint held-out phase-2 performance.
    }
...
\bottomrule
\end{tabular}
\caption{SAC-only checkpoint-ensemble ablations. Lower is better. All variants use saved centralized SAC checkpoints and are therefore evaluated only on the 3-building phase-2 setting.}
...
\begin{figure}[h]
    ...
    \caption{
        Released phase-2 scores for SAC checkpoint-ensemble variants. Bars show the mean over the three released phase-2 splits; points show the corresponding public-dev score. Reference lines mark the earlier best single-checkpoint SAC result and the reproduced CHESCA optimization baseline.
    }
...
\begin{figure}[h]
    ...
    \caption{
        Local-to-held-out comparison for each method. The lighter bar shows the local \texttt{public\_dev} score and the darker bar shows the mean over released \texttt{phase\_2\_online\_eval} splits. SAC variants retain the best absolute held-out performance despite moderate degradation from local scores. PPO shared-DTDE has a smaller absolute gap but starts from a weaker local score, so the smaller gap does not imply better final performance.
    }
    \label{fig:gen_gap}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{
        KPI-level breakdown on the local \texttt{public\_dev} split, normalized relative to RBC for each KPI (RBC $=1$; lower is better). The fourth group is thermal discomfort, where RL methods improve most sharply over RBC. The rightmost group is thermal-resilience loss, which remains a visible residual weakness for several learned policies.
    }
    \label{fig:kpi}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{
        Centralized PPO baseline training curve. The x-axis is training steps (thousands) and the y-axis is mean episode reward (thousands). The curve improves quickly early in training and then plateaus; final algorithm comparisons use the multiseed evaluation scores in Tables~\ref{tab:local} and~\ref{tab:final_matrix}, not this training-reward trace.
    }
    \label{fig:ppo_curve}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{SAC reward-variant ablation on local \texttt{public\_dev}. KPI columns are seed-averaged means (lower is better). ``Cost'' and ``Carbon'' are normalized district totals; ``Discomfort'' is the proportion of occupant-hours with comfort violations; ``1$-$Resilience'' is the thermal-resilience-loss term.}
