...
\begin{figure}[ht]
    ...
    \caption{KPI breakdown on local \texttt{public\_dev}, normalized relative to RBC (RBC $=1$; lower is better). SAC variants sharply reduce thermal discomfort, cost, and carbon; thermal-resilience loss remains the largest residual SAC weakness, while PPO central shows a severe resilience regression.}
    \label{fig:kpi}
\end{figure}
...
The results above establish that architectural portability is the binding constraint for real deployment, SAC substantially outperforms PPO in this continuous-control setting, and RL baselines substantially improve over RBC and close part of the gap to optimization-based winners through careful reward design and evaluation discipline. We discuss these findings and limitations below.
...
\noindent\textbf{TD3 learning-rate screen.}
...
\noindent\textbf{Local-to-held-out comparison.}
...
\clearpage
\paragraph{PPO training diagnostic.} The centralized PPO baseline learning curve rises early and then plateaus, confirming that PPO learned a nontrivial policy before the final multiseed comparisons in Tables~\ref{tab:final_matrix} and~\ref{tab:local}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/ppo_training_curve.png}
    \caption{Centralized PPO baseline training curve.}
    \label{fig:ppo_curve}
\end{figure}
