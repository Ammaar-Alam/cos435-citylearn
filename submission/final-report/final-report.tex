...
\paragraph{KPI breakdown.} The KPI breakdown in Figure~\ref{fig:kpi} shows that the dominant local improvement comes from thermal discomfort, which carries weight 0.30 in the scoring formula. The best SAC variants reduce the discomfort proportion from 0.944 (RBC) to below 0.020. Carbon emissions and electricity cost both fall by roughly 55\%, while thermal-resilience loss remains the main residual weakness. Optimization-based methods such as CHESCA and MPC have a structural advantage here because they can plan over storage and resilience constraints with an explicit horizon rather than learning tradeoffs only from sampled trajectories.
...
\paragraph{Additional result figures.}
