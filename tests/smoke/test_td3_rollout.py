...
from cos435_citylearn.baselines import run_td3
from tests.smoke.helpers import require_benchmark_runtime, require_dataset


@pytest.mark.smoke
def test_centralized_td3_smoke_rollout(tmp_path: Path) -> None:
    require_benchmark_runtime()
    require_dataset()
    payload = run_td3(
        config_path="configs/train/td3/td3_central_smoke.yaml",
    ...
    assert (run_dir / "td3_model.zip").exists()
    assert (run_dir / "vec_normalize.pkl").exists()
    assert (run_dir / "topology.json").exists()
    assert (run_dir / "checkpoint_metadata.json").exists()
    assert (run_dir / "rollout_trace.json").exists()


@pytest.mark.smoke
def test_shared_td3_smoke_rollout(tmp_path: Path) -> None:
    require_benchmark_runtime()
    require_dataset()
    payload = run_td3(
        config_path="configs/train/td3/td3_shared_dtde_smoke.yaml",
        eval_config_path="configs/eval/sac_smoke.yaml",
        output_root=tmp_path / "runs",
        metrics_root=tmp_path / "metrics",
        manifests_root=tmp_path / "manifests",
...
@pytest.mark.smoke
def test_shared_td3_rollout_trace_honors_trace_limit(tmp_path: Path) -> None:
    require_benchmark_runtime()
    require_dataset()
    config = yaml.safe_load(Path("configs/train/td3/td3_shared_dtde_smoke.yaml").read_text())
