...
COMMON_CONTROLLER_STATE_KEYS = {
    ...
    "critic_context_version",
    "normalize_observations",
    "normalize_critic_observations",
    "normalize_rewards",
    "normalize_advantage",
    "target_kl",
    "policy_state_dict",
...
def safe_load_mappo_checkpoint_payload(checkpoint_path: str | Path) -> dict[str, Any]:
    try:
        payload = torch.load(Path(checkpoint_path), map_location="cpu", weights_only=True)
    except Exception as exc:
        raise ValueError(f"failed to safely load MAPPO checkpoint: {exc}") from exc

    validate_mappo_checkpoint_payload_structure(payload)
    return payload


def validate_mappo_checkpoint_payload_structure(payload: Any) -> None:
    if not isinstance(payload, dict):
        raise ValueError("MAPPO checkpoint payload must be a mapping")

    missing = sorted(REQUIRED_CHECKPOINT_KEYS - set(payload))
    if missing:
        raise ValueError(f"MAPPO checkpoint payload missing required keys: {', '.join(missing)}")

    format_version = payload["format_version"]
    if format_version not in SUPPORTED_FORMAT_VERSIONS:
        raise ValueError(
            f"unsupported MAPPO checkpoint format_version: {format_version}. "
            f"supported versions: {sorted(SUPPORTED_FORMAT_VERSIONS)}"
        )

    if payload["algorithm"] != "mappo":
        raise ValueError("checkpoint payload is not a MAPPO checkpoint")

    if payload["control_mode"] != "shared_ctde":
        raise ValueError(
            f"unsupported MAPPO checkpoint control_mode: {payload['control_mode']}. "
            "MAPPO only supports control_mode=shared_ctde."
        )

    if not isinstance(payload["observation_names"], list) or not isinstance(
        payload["action_names"], list
    ):
        raise ValueError("MAPPO checkpoint schema metadata is malformed")

    controller_state = payload["controller_state"]
    if not isinstance(controller_state, dict):
        raise ValueError("MAPPO checkpoint controller_state must be a mapping")

    missing_state = sorted(COMMON_CONTROLLER_STATE_KEYS - set(controller_state))
    if missing_state:
        raise ValueError(
            "MAPPO checkpoint controller_state missing required keys: "
            + ", ".join(missing_state)
        )

    if controller_state["controller_type"] != "forecast_augmented_mappo":
        controller_type = controller_state["controller_type"]
        raise ValueError(f"unknown MAPPO checkpoint controller type: {controller_type}")

    if int(controller_state["shared_context_dimension"]) != SHARED_CONTEXT_V2_DIMENSION:
        raise ValueError(
            f"MAPPO checkpoint shared_context_dimension must be {SHARED_CONTEXT_V2_DIMENSION}"
        )
    if controller_state.get("shared_context_version") != "v2":
        raise ValueError("MAPPO checkpoint shared_context_version must be 'v2'")
    if controller_state.get("critic_context_version") != CENTRALIZED_CRITIC_CONTEXT_VERSION:
        raise ValueError(
            "MAPPO checkpoint critic_context_version must be "
            f"{CENTRALIZED_CRITIC_CONTEXT_VERSION!r}"
        )


def validate_mappo_checkpoint_runner_compatibility(
    ...
    *,
    ...
    if mismatches and not allow_cross_reward_eval:
        ...
        raise ValueError(
            "MAPPO checkpoint runtime metadata is incompatible with runner config; "
...
def validate_mappo_checkpoint_env_compatibility(
    checkpoint_payload: dict[str, Any],
    *,
    ...
    if not checkpoint_observation_names or not env_observation_names:
        raise ValueError("MAPPO checkpoint requires non-empty observation schema on both sides")
    ...
    if any(list(b) != ckpt_per_building_obs for b in checkpoint_observation_names):
        raise ValueError("MAPPO checkpoint has inconsistent per-building observation schemas")
    if any(list(b) != env_per_building_obs for b in env_observation_names):
        raise ValueError(
            ...
            "MAPPO requires identical buildings"
        )
    if list(ckpt_per_building_obs) != list(env_per_building_obs):
        raise ValueError(
            "MAPPO checkpoint per-building observation schema does not match target env; "
            "the two datasets expose different building features."
        )

    if not checkpoint_action_names or not env_action_names:
        raise ValueError("MAPPO checkpoint requires non-empty action schema on both sides")
    ...
    if any(list(b) != ckpt_per_building_act for b in checkpoint_action_names):
        raise ValueError("MAPPO checkpoint has inconsistent per-building action schemas")
    ...
    if list(ckpt_per_building_act) != list(env_per_building_act):
        raise ValueError("MAPPO checkpoint per-building action schema does not match target env.")
