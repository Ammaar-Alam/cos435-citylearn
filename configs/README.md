...
- `train/` holds method configs for RBC, PPO, SAC, TD3, and MAPPO
...
- `train/mappo/*.yaml` drives the shared CTDE MAPPO runner
- `train/mappo/*_smoke.yaml` is intentionally tiny config for test coverage only
...
- `eval/mappo_smoke.yaml` is the capped MAPPO smoke-eval path

RBC, PPO, SAC, TD3, and MAPPO configs are live runner inputs. They are launchable through
...
portable variants used for released phase-3 cross-topology evaluation, alongside
shared TD3 and shared CTDE MAPPO.
